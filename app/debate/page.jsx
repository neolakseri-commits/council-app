  ...
  const startDebate = () => {
    ...
    archetypes.forEach((arch, idx) => {
      setTimeout(() => {
        ...
        setAgentArguments((prev) => ({ ...prev, [arch]: randomArg }))
      }, delay)
      delay += 1000
    })
      ...
      {(phase === 'debate' || phase === 'verdict') && (
        <div className={styles.debatePhase}>
          ...
          <div className={styles.debateArguments}>
            <div className={`${styles.argument} ${agentArguments.reckless ? styles.active : ''}`}>
              <div className={styles.archType}>🎯 RECKLESS</div>
              <p>{agentArguments.reckless || '...'}</p>
              {agentArguments.reckless && <div className={styles.confidence}>78%</div>}
            </div>

            <div className={`${styles.argument} ${agentArguments.cautious ? styles.active : ''}`}>
              <div className={styles.archType}>⚠️ CAUTIOUS</div>
              <p>{agentArguments.cautious || '...'}</p>
              {agentArguments.cautious && <div className={styles.confidence}>62%</div>}
            </div>

            <div className={`${styles.argument} ${agentArguments.analyst ? styles.active : ''}`}>
              <div className={styles.archType}>📊 ANALYST</div>
              <p>{agentArguments.analyst || '...'}</p>
              {agentArguments.analyst && <div className={styles.confidence}>71%</div>}
            </div>

            <div className={`${styles.argument} ${agentArguments.dreamer ? styles.active : ''}`}>
              <div className={styles.archType}>💫 DREAMER</div>
              <p>{agentArguments.dreamer || '...'}</p>
              {agentArguments.dreamer && <div className={styles.confidence}>75%</div>}
            </div>
          </div>
        </div>
      )}
